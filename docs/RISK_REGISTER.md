...
| **ASYNC-3** | Async | ✅ Done | WS + full-rate polling both ran (realtime cost, no load savings). Now **substitutive**: both `OrderStatus.vue` + `OwnerOrders.vue` poll via a self-rescheduling timer that reads the WS `connectionState` — 60s safety-net when live, else the 15s primary → ~4× fewer HTTP polls when connected | M |
| **ASYNC-4** | Async | ◑ Code done · owner-gated | `acks_late` redelivery double-sends — **dedupe + code-level retry-exhaustion alert both shipped** (`_NotificationTask.on_failure`, commit `6e9c3be`, tested). Only the broker **dead-letter exchange** remains — genuine infra work, not a code fix | S/M |
...
| **STRUCT-2** | Structure | ◑ Code done · owner-gated | 216 migrations, `Order` field sprawl, no squashing → slow per-schema deploys. **Squash-prep done**: the 5 one-time-backfill `RunPython`s marked `elidable=True` (behavior-neutral now), the 6th (`menu/0005` default-SuperCategory seed) deliberately kept, and a [squash runbook](STRUCT-2_squash_runbook.md) written (handles the 3 `AddIndexConcurrently` `atomic=False` migrations → squash `menu` only to 0059). The squash itself is an **owner/release-boundary** step (needs a real multi-tenant DB to validate; marginal value until the MULTITENANCY-1 scale ceiling); `Order` decomposition is STRUCT-1 | M |
...
Plus the **Marketplace checkout drawer decomposition (part 1)** — the ~400-line checkout drawer in
`MarketplaceMenuPage.vue` is being split into child components; part 1 lifts the four low-risk, non-money
sub-parts: `MarketplaceCheckoutCartItems` (cart line list + qty steppers; emits decrement/increment,
`cart` + mutations stay in the parent), `MarketplaceCheckoutFulfillmentType` (pickup/delivery selector,
`v-model:fulfillment-type`), `MarketplaceCheckoutSchedule` (ASAP/later + datetime, two v-models), and
`MarketplaceCheckoutLoyalty` (redeem toggle + earn line, `v-model:use-loyalty`; loyalty math stays). No
`placeOrder` / wallet / tip / prepay / validation moved. 4 components, 20 test cases. **Part 2** lifts the
delivery-details block (`MarketplaceCheckoutDelivery` — saved-address picker, address, save-for-later,
