...
| C12 ✅ | **Super-app identity + onboarding/role-switching pass (frontend).** **Shipped (commit 4fbc985):** de-restaurant-ified ~38 user-facing strings on the discovery + generic surfaces (Marketplace/Directory/cart/mkt-menu/layout fallbacks/publish/home/legal) in en+fr+ar (lockstep with the `messages.js` verifier aggregate) so shops/pharmacies aren't mislabelled "restaurants"; driver tile reframed "Drive & earn". **Role-switching:** driver-mode switch promoted into `CustomerLayout` header (was buried), `WaiterLayout` always shows Sign out for pure staff, `SuperAppHub` "Open your business on Kepoli" partner CTA → `/get-started`. **Onboarding/first-run:** DriverPage richer pending state + one-time approved welcome, WaiterPage first-run banner, CustomerAuthModal one-time post-signup welcome via centralized `finishAuth()`, owner wizard redirects out if already published. **Admin chrome:** new `AdminLayout.vue` (skip-link + focusable `<main>` + nav over the 9 admin routes + sign out), router wraps admin routes in it (guards preserved). `useSeoMeta` titles platform-host pages with `PLATFORM_NAME`. Constraints held: per-tenant identity sacred, no session merge, no Dish→Item rename. Gates: verify:i18n + lint(0 warn) + build + 137 vitest. | L | (this request) |
| C13 ✅ | **Per-service account model (P0–P4) — COMPLETE.** One global account + per-service profiles + global wallet. Roadmap: **`KEPOLI_ACCOUNT_ARCHITECTURE.md`** (red-teamed). L1 global (identity, one wallet, addresses), L2 `CustomerServiceProfile (customer, vertical)`, L3 `vertical` tag on `CustomerOrderRef` + `WalletTransaction` (metadata only — balance stays one global field). **Progress:** ✅ **P0** taxonomy seam (`accounts/verticals.py` + `lib/verticals.js`; hub gates on `enabled_verticals`; commit 7e0f1bf). ✅ **P1a** `CustomerOrderRef.vertical` (signal + backfill; migration 0055; b855999). ✅ **P1b** `WalletTransaction.vertical` (auto-derive from tenant_id in credit/debit/transfer + explicit rides/courier/driver; migration 0056; backfill; balance untouched; 6b15f8a). ✅ **P2** `CustomerServiceProfile` (migration 0057; e105656) + per-vertical promo suppression at the campaign/winback audience level (reconciled w/ `CustomerTenantOptOut`, suppress-if-either; 1e17c60) + `GET /api/customer/services/` per-vertical activity summary (c6f8908). ✅ **P3** `?vertical=` scoped reads on `/customer/orders/all/` + `/customer/wallet/` (+ `spend_by_vertical`) + GET/PATCH `/customer/service-profiles/` (31596b9); P3c reservation FK deferred (non-blocking). 🟡 **P4** ✅ P4a FE service-context (customer store `enabledVerticals`/`isVerticalEnabled` getters + router vertical-gating of /ride·/send-package, fail-open; 0059bd9). ✅ P4b account UI (order-history vertical filter + wallet `spend_by_vertical` breakdown + per-service notif-prefs UI; a46e4a0) + ✅ P4c hub per-service activity badges (3a817e6). **ALL PHASES SHIPPED.** Remaining deferrals (non-blocking): P3c reservation `Customer` FK; full per-service *dashboards* (the filter/breakdown deliver the scoped views without a route split). Deploy: backfill is automatic (data migration 0058). **Post-ship completeness audit (multi-agent) → fixed (b482b50): 3 wallet vertical mislabels (staff-payment missing tenant_id; driver delivery/no-show EARNING → driver not food); EARNING backfill → driver; CustomerServicesView driver count from delivered jobs; default_address wired into the prefs endpoint (IDOR-guarded); get_or_create_for vertical validation.** Accepted/deferred (minor/intentional): voucher TOPUP stays global-null; send_campaign_push_sync per-vertical mute is audience-level only (all callers pre-filter); default_address has API but no FE picker / checkout consumption yet; order-filter empty-state polish. | XL | KEPOLI_ACCOUNT_ARCHITECTURE.md |
| C14 ✅ | **Super-app shop/pharmacy hardening (5-area audit → fixes).** Audited the full consumer ordering journey for non-restaurant verticals. **Fixed all P0/P1:** (capability bypasses, 08698f4) P0 Cart dine-in not gated by `capabilities.dine_in` (→ `?table=` on a shop = order with no payment); P1 bare `fulfillment_type=table` skipped the dine-in gate; P1 `delivery_enabled` not enforced at placement (both order paths); P1 CustomerLayout "Reserve" tab not capability-gated; P2 `StaffFireCourseView` kitchen gate. (vocabulary, 6d3fcd5) neutralized ~28 consumer-facing restaurant/kitchen/dishes leak strings (mktMenu/mktOrderStatus/orderStatus/menu/category/dishPage) en+fr+ar lockstep. (discovery, 934d927) Directory business-type lens (Food/Shops/Pharmacy via `verticalForBusinessType`). **Deferred P2 (cosmetic):** cart "restaurant closed" strings + backend error text; delivery-button table icon; `businessIcon` fallback triplication; grocery/bakery/retail not in services hub; `MarketplaceMenuView` business_type exposure + full `useVocabulary` noun-wiring (Dish→Product). Gates green throughout. | L | (audit-driven) |
| C15 | **`Dish→Item` schema rename.** Held until a PSP-enabled retail tenant proves demand; 204 backend symbols + 53 frontend files + a `migrate_schemas` pass = high blast radius for zero current payoff. Rehearse on a clone (ARCH §3). | XL | BACKLOG:38; ARCH §3 |
