...
| C13 | **`Dish→Item` schema rename.** Held until a PSP-enabled retail tenant proves demand; 204 backend symbols + 53 frontend files + a `migrate_schemas` pass = high blast radius for zero current payoff. Rehearse on a clone (ARCH §3). | XL | BACKLOG:38; ARCH §3 |
