...
| C13 | **Per-service account model (P0–P4).** One global account + per-service profiles + global wallet. Roadmap: **`KEPOLI_ACCOUNT_ARCHITECTURE.md`** (PROPOSED, red-teamed). Three layers: L1 global (identity, one wallet, addresses), L2 `CustomerServiceProfile (customer, vertical)`, L3 `vertical` tag on `CustomerOrderRef` + `WalletTransaction` (metadata only — balance stays one global field). P0 taxonomy seam → P1 tag data → P2 profiles → P3 scoped reads → P4 FE contexts. All additive/non-breaking. | XL | KEPOLI_ACCOUNT_ARCHITECTURE.md |
