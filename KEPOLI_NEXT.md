...
|---|---|---|---|
| A1 | **OPS-5e security batch (one batch).** Driver cash-out: bind code lookup to the driver's intended tenant + per-user throttle + lock after N bad attempts (mirror delivery-code lockout). MarketplaceOrderStatusView IDOR: gate the financial body on session ownership (or per-order access token). Wallet idempotency: server-namespace keys (tenant_id+endpoint) + assert stored row amount/tenant/customer before replay. Push-sub hijack: scope `update_or_create` to (customer_id, endpoint). Rating fraud: require ownership + throttle. | L | OPS-5e (BACKLOG:157-204) |
| A2 | **`Plan.price`/`currency`/`billing_period` migration** + wire `pricing.js` + marketing page. *(Gated on owner numbers — 2.0 step 2.)* | M | OPS-6 pricing scaffold; ARCH §4d decision-gated |
| A3 | **PSP top-up seam, dormant behind a flag.** `CustomerWallet.vue` + top-up-intent endpoint + `StripeWebhookView` → idempotent credit. No live keys. | L | ARCH Phase 3; BACKLOG "Stripe wallet top-up seam" |
| A4 | **Marketplace COD-on-handover** (port `_cod_eligible` to `MarketplacePlaceOrderView`). *Pre-PSP GMV.* | M | (new — from monetization lens) |
| A5 | **Marketplace commission fix** (per-tenant rate + `Order.commission_rate_applied` snapshot + consistent basis + tz month). *(Gated on owner default rate.)* | M | BACKLOG:373-384 |
| A6 | **Redis `requirepass`/ACL** + fail-fast if broker URL unauthenticated in prod. Defense-in-depth before real money flows on a no-auth Redis holding sessions/idempotency/channel state. | S | OPS-5d Celery-RCE note (mitigation) |
| A7 | **Stand up durable prod infra: Redis + Celery worker, fail-fast if absent.** Today prod silently degrades when `REDIS_URL`/`CELERY_BROKER_URL` are unset: cache → in-process `LocMemCache`, channel layer → `InMemoryChannelLayer` (broadcasts don't cross workers, so live order/paid updates miss multi-worker sockets), and the task queue runs inline in a daemon thread (a crash/restart drops in-flight notifications — no retry, no durability). Set `REDIS_URL` + `CELERY_BROKER_URL` (same Redis) + run a worker + Beat, and **assert their presence at boot when `DEBUG=False`** so a misconfigured deploy hard-fails instead of silently losing money-adjacent events. This is a hard prerequisite for accepting real money on multi-worker prod. | M | Scale lens P1 (settings.py:201-296, durability blocker) |
...
| B5 | **Real table-state model** (open/occupied/dirty/reserved) + transfer-tab + merge-tabs, built on `TableLink`/`TableSection`. Also fixes the `table_slug` vs `table_label` rename-splits-history bug. | L | BACKLOG:358; Restaurant lens |
| B6 | **Referral mechanic** (give-X-get-X via the idempotent wallet/loyalty credit). Loyalty-point referral works today with **zero PSP**; full-value wallet referral lands when top-up flips on. | L | CX lens |
| B7 | **Customer re-order rail + loyalty progress nudge** (reuse `accounts/views.py:696` cross-restaurant history). Table-stakes vs Glovo/Jahez. | M | CX lens |
| B8 | **Marketplace cross-schema N+1 kill.** Denormalize `rating_avg`/`rating_count` onto the public-schema `Profile` via post_save + backfill; `DirectoryView`/`MarketplaceView` then read from the fetched queryset (100+ schema hops → 1 query). The dominant public-endpoint scale cliff. | M | BACKLOG:446; Scale lens |
...
| B10 | **`perm_void` (+ comp/refund) split out of `perm_manage_orders`.** Loss-prevention: today any waiter who can settle can also void. One boolean + endpoint gate + `OwnerStaffPage`. | S | Restaurant lens; OPS_AUDIT Theme 8 |
| B11 | **Renewal-ready billing surface** (default `invoice_amount` from `Plan.price`; renewal-due list in AdminConsole; pre-lapse reminder email). Protects the recurring line pre-PSP. | M | Monetization lens |
| B12 | **OPS-6c polish (privacy-first items).** noindex personal/transactional routes (cart/account/order-status currently emit `index,follow` — privacy leak); sitemap.xml + `Sitemap:` directive; hreflang en/fr/ar; MenuItem/Breadcrumb structured data; DishCard ARIA; skip-link focus on Customer+Owner layouts. | M | OPS-6c (BACKLOG:75-117) |
| B13 | **Scale sweep:** customer CRM segment predicate → SQL (`menu/views.py:8674`); phone search → pg_trgm GIN or digits[-9:] (the shipped btree is dead for icontains); non-blocking index migrations (`AddIndexConcurrently`); residual Z-report/export N+1 + `OrderItem.voided_at` partial index. | M | BACKLOG:437-465; Scale lens |
...
| C6 | **Full data-subject tooling** (self-serve export + erasure/anonymization endpoints + consent records). The *decision* on jurisdiction/retention scope and the minimum baseline (published privacy policy + retention rule + a manual export/erasure runbook) are pulled forward into the Phase-A privacy gate (§4) — because Phase A is when real PII + PSP/PCI scope first land. What remains here is the automated tooling, which can phase after the manual baseline is in place. | L | Security lens (P1 decision, gated in §4; tooling P2) |
| C7 | **`Dish→Item` schema rename.** Held until a PSP-enabled retail tenant proves demand; 204 backend symbols + 53 frontend files + a `migrate_schemas` pass = high blast radius for zero current payoff. Rehearse on a clone (ARCH §3). | XL | BACKLOG:38; ARCH §3 |
...
| R5 | **Scope dilution across 5 advertised verticals** vs the restaurant-first directive. ~3,600 lines of parked ride/courier surface carry live security obligations with no backend kill-switch. | Medium | Decision gate (vertical focus): shelve rides, keep courier, park shops; add a `VERTICALS_ENABLED` settings gate so "parked" actually shrinks the attack surface, not just hides the tile. |
