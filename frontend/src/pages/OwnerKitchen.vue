<template>
  ...
  >
    ...
    <div v-else class="kitchen-grid" role="list" :aria-label="t('kitchen.activeCount', { n: activeOrders.length })">
      <OwnerKitchenOrderCard
        ...
        :headline-color-class="headlineColorClass"
        :order-headline="orderHeadline"
        :chip-class="chipClass"
        :elapsed-badge-class="elapsedBadgeClass"
        :elapsed-minutes="elapsedMinutes"
        :elapsed-label="elapsedLabel"
        :time-ago="timeAgo"
        :format-scheduled-for="formatScheduledFor"
        :kitchen-due-soon="kitchenDueSoon"
        :dj-chip-class="djChipClass"
        :dj-chip-label="djChipLabel"
        :order-ready-count="orderReadyCount"
        :is-item-held="isItemHeld"
        :lowest-held-course="lowestHeldCourse"
        :has-unready-items="hasUnreadyItems"
        :all-items-ready="allItemsReady"
        :action-btn-class="actionBtnClass"
        :action-label="actionLabel"
        @toggle-item="toggleItem"
        @fire-course="fireCourse"
        @mark-all-ready="markAllReady"
        @advance="advance"
  ...
  <!-- 86 Board modal (contract 7) — RISK FE-2 -->
  <OwnerKitchen86Board
    v-model:search="eightySixSearch"
    :open="eightySixOpen"
    :dishes="eightySixFiltered"
    :fetching="eightySixFetching"
    :toggling-id="eightySixTogglingId"
    @close="eightySixOpen = false"
    @toggle="toggle86Dish"
  />
</template>
...
import OwnerKitchen86Board from "../components/OwnerKitchen86Board.vue";
