      ...
      >
        <div ref="leadDialogRef" role="dialog" aria-modal="true" aria-labelledby="customer-lead-contact-dialog-title" class="w-full max-w-lg rounded-t-3xl border border-slate-700/80 bg-slate-950 p-4 shadow-2xl shadow-black/50 sm:rounded-2xl sm:p-5">
          ...
          <form class="space-y-3" novalidate @submit.prevent="submitLead">
            ...
            <label class="space-y-1 text-sm text-slate-200">
              {{ t("common.email") }}
              <input v-model.trim="form.email" type="email" class="ui-input" :class="fieldClass('email')" inputmode="email" autocomplete="email" spellcheck="false" :aria-invalid="errors.email ? 'true' : undefined" aria-describedby="lead-email-error" @input="clearError('email')" />
