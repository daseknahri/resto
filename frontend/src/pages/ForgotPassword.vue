    ...
    <div class="ui-auth-stage">
      ...
      <div class="ui-auth-card ui-reveal space-y-6" :style="{ '--ui-delay': '80ms' }">
        <div class="space-y-2 text-center">
          ...
          <h2 class="ui-display text-2xl font-semibold text-white tracking-tight">{{ t("forgotPassword.title") }}</h2>
          <p class="text-sm text-slate-300 leading-relaxed">{{ t("forgotPassword.description") }}</p>
        </div>
        ...
        <form class="space-y-5" novalidate @submit.prevent="submit">
          <label class="block space-y-1.5 text-sm font-medium text-slate-200">
            ...
            />
            <p v-if="identifierError" id="forgot-identifier-error" class="mt-1 text-xs text-red-300" role="alert">{{ identifierError }}</p>
          </label>
        ...
        <div class="ui-section-band flex flex-wrap items-baseline gap-x-1 text-xs text-slate-300">
          ...
          <RouterLink
            class="text-[var(--color-secondary)] underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 focus-visible:rounded-sm focus-visible:underline"
            :to="signinLink"
          >{{ t("forgotPassword.signInLink") }}</RouterLink>
        </div>
      </div>
    </div>
  </div>
