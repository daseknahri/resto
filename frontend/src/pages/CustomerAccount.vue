      ...
      <div id="customer-account-tabpanel" class="px-3 py-3 pb-28 space-y-3" role="tabpanel" :aria-labelledby="'tab-' + activeTab">
        ...
        <template v-else-if="activeTab === 'orders'">
          <CustomerAccountOrders
            :vertical-filter-options="VERTICAL_FILTER_OPTIONS"
            :selected-vertical="selectedVertical"
            :order-search="orderSearch"
            :marketplace-orders="marketplaceOrders"
            :filtered-marketplace-orders="filteredMarketplaceOrders"
            ...
            :orders-has-more="ordersHasMore"
            :loading-more-orders="loadingMoreOrders"
            :recent-orders="cart.recentOrders"
            :active-statuses="ACTIVE_STATUSES"
            :expanded-orders="expandedOrders"
            :cancelling-order-number="cancellingOrderNumber"
            :vertical-svc-labels="VERTICAL_SVC_LABELS"
            ...
            @view-receipt="receiptOrder = $event"
            @reorder="reorder"
        ...
        <template v-else-if="activeTab === 'reviews'">
          <CustomerAccountReviews
            ...
            :submitted-reviews="submittedReviews"
            :reviews-avg-score="reviewsAvgScore"
            :review-hover="reviewHover"
            :review-drafts="reviewDrafts"
            :submitting-review="submittingReview"
            :review-score-labels="reviewScoreLabels"
            @hover="setHover"
            @draft-score="setDraftScore"
            @draft-comment="setDraftComment"
            @submit="submitReview"
          />
        </template>
        ...
        <template v-else-if="activeTab === 'profile'">
          <CustomerAccountProfile
            ...
            :show-email-input="showEmailInput"
            :editable-email="editableEmail"
            :saving-email="savingEmail"
            :email-error="emailError"
            :editable-birthday="editableBirthday"
            :saving-birthday="savingBirthday"
            :saved-addresses="savedAddresses"
            :loading-addresses="loadingAddresses"
            :adding-address="addingAddress"
            :addr-form="addrForm"
            :addr-error="addrError"
            ...
            :addr-delete-error="addrDeleteError"
            :locale-configured="localeConfigured"
            :selected-locale="selectedLocale"
            :saving-locale="savingLocale"
            :locale-label-current="localeLabelCurrent"
            :currency-options="currencyStore.available"
            :selected-currency="currencyStore.selected"
            :saving-prefs="savingPrefs"
            :push-supported="pushSupported"
            :push-enabled="pushEnabled"
            :push-subscribed="pushSubscribed"
            :push-loading="pushLoading"
            :service-profiles-open="serviceProfilesOpen"
            :service-profiles="serviceProfiles"
            :loading-service-profiles="loadingServiceProfiles"
            :service-profiles-error="serviceProfilesError"
            :saving-service-profile="savingServiceProfile"
            :enabled-verticals="customerStore.enabledVerticals"
            :vertical-svc-labels="VERTICAL_SVC_LABELS"
            :exporting-data="exportingData"
            :requesting-erasure="requestingErasure"
            :erasure-confirm-visible="erasureConfirmVisible"
            :erasure-blocked-msg="erasureBlockedMsg"
            @update-editable-name="editableName = $event"
            @save-name="saveName"
            @update-editable-email="editableEmail = $event"
            @open-email-input="openEmailInput"
            @save-email="saveEmail"
            @cancel-email-input="cancelEmailInput"
            @update-editable-birthday="editableBirthday = $event"
            @save-birthday="saveBirthday"
            @add-phone="showAddPhone = true"
            @start-adding-address="addingAddress = true"
            @cancel-adding-address="addingAddress = false; addrForm.label = ''; addrForm.address = ''; addrError = ''"
            @add-address="addAddress"
            @addr-label-input="addrForm.label = $event"
            @addr-address-input="addrForm.address = $event"
            @toggle-edit-address="(addr) => (editingAddressId === addr.id ? (editingAddressId = null) : startEdit(addr))"
            @save-edit-address="saveEdit"
            @cancel-edit-address="editingAddressId = null"
            @edit-label-input="editForm.label = $event"
            @edit-address-input="editForm.address = $event"
            @start-delete-address="(id) => { deletingAddressId = id; editingAddressId = null; addrDeleteError = ''; }"
            @cancel-delete-address="deletingAddressId = null; addrDeleteError = ''"
            @delete-address="deleteAddress"
            @set-locale="setLocale"
            @edit-locale="localeConfigured = false"
            @set-currency="currencyStore.setCode"
            @save-pref="savePref"
            @push-subscribe="pushSubscribe"
            @toggle-service-profiles="serviceProfilesOpen = !serviceProfilesOpen; !serviceProfiles || Object.keys(serviceProfiles).length === 0 ? fetchServiceProfiles() : null"
            @save-service-pref="saveServicePref"
            @download-my-data="downloadMyData"
            @start-erasure="erasureConfirmVisible = true"
            @request-erasure="requestErasure"
            @cancel-erasure="erasureConfirmVisible = false; erasureBlockedMsg = ''"
          />
        </template>
...
import CustomerAccountProfile from '../components/CustomerAccountProfile.vue';
import CustomerAccountReservations from '../components/CustomerAccountReservations.vue';
import CustomerAccountReviews from '../components/CustomerAccountReviews.vue';
...
// Autofocus-on-open now lives inside CustomerAccountProfile (the input itself
// moved there with the rest of the profile markup) — it watches
// showEmailInput and focuses its own local template ref.
