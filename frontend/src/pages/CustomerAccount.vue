...
// ── Customer-confirmed charges ─────────────────────────────────────────────────
// A restaurant can request an above-threshold wallet charge that THIS customer must
// approve. Poll for pending requests and surface an approve/decline modal.
const pendingCharges = ref([]);
const chargeBusy = ref(null);  // id of the request being approved/declined
const chargeError = ref('');
let chargePollTimer = null;
const CHARGE_POLL_MS = 5000;
...
const startChargePolling = () => {
  stopChargePolling();
  fetchChargeRequests();
  chargePollTimer = setInterval(fetchChargeRequests, CHARGE_POLL_MS);
};
const stopChargePolling = () => {
  if (chargePollTimer) { clearInterval(chargePollTimer); chargePollTimer = null; }
};
onBeforeUnmount(stopChargePolling);
