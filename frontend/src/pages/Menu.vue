  ...
  <div class="space-y-3 px-4 py-3 pb-32 sm:pb-8 ui-safe-bottom">
    <header class="ui-glass ui-reveal overflow-hidden p-3.5 md:p-4">
      <div class="space-y-1.5">
        <p class="ui-kicker">Menu browsing</p>
        <h1 class="ui-display text-xl font-semibold tracking-tight text-white md:text-2xl">{{ tenantName }}</h1>
        <p class="text-sm text-slate-300">
          Explore categories and tap any dish to view details, options, and pricing.
        </p>
      </div>

      <div class="mt-3 ui-divider"></div>
      <div class="mt-3 flex flex-wrap gap-2">
        <span class="ui-chip">{{ categories.length }} categories</span>
        <span class="ui-chip">{{ totalDishes }} dishes</span>
    ...
    <section class="ui-panel ui-reveal ui-surface-lift space-y-3 p-4" style="--ui-delay: 50ms">
      <div class="grid gap-3 md:grid-cols-[1fr,auto] md:items-center">
        <div class="relative">
          <input v-model.trim="search" class="ui-input pr-12" placeholder="Search category..." />
          <button
            v-if="search"
      ...
      <p class="text-xs text-slate-500">Tip: choose clear category images to increase customer taps and conversion.</p>
    </section>
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <div v-if="!menu.loading && !categories.length" class="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 text-slate-300 sm:col-span-2">
        <p class="font-semibold text-slate-100">No matching categories.</p>
        <p class="mt-1 text-sm text-slate-400">Try a different search term or check back later.</p>
      </div>
    ...
    >
      <div>
        <p class="text-xs text-slate-400">Cart</p>
        <p class="font-semibold">{{ cart.count }} item(s)</p>
      </div>
      <p class="text-base font-semibold text-[var(--color-secondary)]">{{ formatPrice(cart.total, cartCurrency) }}</p>
    </RouterLink>
  </div>
</template>
...
const categories = computed(() => {
  ...
  const filtered = menu.categories.filter((cat) => {
...
onMounted(async () => {
  if (!menu.categories.length) await menu.fetchCategories();
