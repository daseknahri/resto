<template>
  ...
  <div
    v-if="fulfillmentType === 'delivery' && deliveryMinGap > 0"
  ...
  <div
    v-if="fulfillmentType === 'delivery' && !isAuthenticated"
...
<script setup>
// The pre-submit warning banners of MarketplaceMenuPage.vue's checkout drawer,
// extracted as a DISPLAY-ONLY FRAGMENT child (RISK FE-2) — the final tidy of the
// checkout-drawer split. Three independently-gated alert/status banners: minimum-order
// not met (delivery), restaurant closed, and guest-must-sign-in (delivery). No
// props are mutated; the guest banner's "Sign in" button emits `signIn` so the parent
// can open its CustomerAuthModal (the same modal the auth_required order error opens).
// The parent still derives every condition (deliveryMinGap, restaurant.is_open, auth)
// and the submit button + placeOrder stay in the parent. A fragment (no wrapper root)
// so the banners keep their place in the drawer body's vertical rhythm.
...
defineProps({
  /** Fulfillment type (form.fulfillment_type) — gates the delivery-only banners. */
  fulfillmentType: { type: String, default: 'pickup' },
  /** Amount still needed to hit the delivery minimum (deliveryMinGap). */
  deliveryMinGap: { type: Number, default: 0 },
  /** Whether the restaurant is currently closed (restaurant && !restaurant.is_open). */
  isClosed: { type: Boolean, default: false },
  /** Whether a customer is signed in (customerStore.isAuthenticated). */
  isAuthenticated: { type: Boolean, default: false },
  /** Price formatter (fmtPrice) for the minimum-order copy. */
