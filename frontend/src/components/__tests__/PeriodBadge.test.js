...
describe("PeriodBadge", () => {
  ...
  it("shows a + sign and emerald color for a positive change", () => {
    ...
    expect(w.find("span").classes()).toContain("text-[var(--color-success)]");
  });
  ...
  it("shows a minus sign and red color for a negative change", () => {
    ...
    expect(w.find("span").classes()).toContain("text-[var(--color-danger-soft)]");
  });
  ...
  it("treats exactly zero as a neutral, signless badge", () => {
    ...
    expect(w.find("span").exists()).toBe(true); // 0 is a real value, still renders
