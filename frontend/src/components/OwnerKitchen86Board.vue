...
<script setup>
// 86-board modal of OwnerKitchen.vue, extracted as a standalone child (RISK FE-2,
// entangled tier). This is the presentational shell: it renders the search box,
// the (already filtered + stably-sorted) dish list and the per-dish availability
// toggles, and forwards intent via emits.
//
// The complexity stays in the parent (OwnerKitchen.vue): it owns the dish data
// (`eightySixDishes`), the fetch (`fetch86Dishes`), the availability toggle API
// (`toggle86Dish`, which mutates dish.is_available in place — the same object refs
// this list renders, so the button updates reactively), and — crucially — the
// snapshotted sort-order Map (`_eightySixOrderKey`) + the `eightySixFiltered`
// computed. The sort order is deliberately snapshotted per open/refetch (NOT
// recomputed per toggle) so a row never jumps mid tap-sequence; that logic is
// untouched here. The parent passes the computed result in as `dishes`, and the
// search string round-trips via `v-model:search` (defineModel) so the parent's
// filter computed stays reactive.
...
defineProps({
  /** Whether the board is open (parent-owned eightySixOpen). */
  open: { type: Boolean, default: false },
  /** The already filtered + stably-sorted dishes (parent's eightySixFiltered). */
