...
class CustomerOrderStatusViewTests(SimpleTestCase):
    ...
    def test_unknown_order_number_returns_404(self, objects_mock):
        objects_mock.filter.return_value.prefetch_related.return_value.first.return_value = None
    ...
    def test_found_order_returns_200_with_full_shape(self, objects_mock):
        objects_mock.filter.return_value.prefetch_related.return_value.first.return_value = _make_order()
        resp = self._get()
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        for field in (
            "order_number", "status", "fulfillment_type", "table_label",
            "customer_name", "delivery_address", "delivery_location_url",
    ...
    def test_customer_phone_not_in_response(self, objects_mock):
        ...
        objects_mock.filter.return_value.prefetch_related.return_value.first.return_value = order
        resp = self._get()
