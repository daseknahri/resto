...
class RideEstimateViewTests(SimpleTestCase):
    ...
    def test_valid_estimate(self, mock_estimate, _throttle):
        mock_estimate.return_value = {"distance_km": 3.2, "fare": Decimal("23.20")}
        req = self._post({
...
class RideCreateViewTests(SimpleTestCase):
    ...
    @patch("accounts.models.Customer.objects")
    @patch("accounts.ride_service.estimate_ride")
    def test_duplicate_active_ride_returns_409(self, mock_est, mock_cust_objs, mock_ride_objs, _throttle):
    ...
    @patch("accounts.models.Customer.objects")
    @patch("accounts.ride_service.estimate_ride")
    def test_successful_creation_returns_201(self, mock_est, mock_cust_objs, mock_ride_objs, _throttle):
...
class SettleRideTests(SimpleTestCase):
    ...
    def test_insufficient_balance_flips_to_cash(self, mock_cfg, mock_debit):
        ...
        self.assertIn("payment_method", call_kwargs.get("update_fields", []))
...
class UpdateFieldsHouseRuleTests(SimpleTestCase):
    ...
    def test_settle_ride_insufficient_funds_update_fields(self):
        """InsufficientFunds path: save(update_fields=['payment_method'])."""
        from accounts.ride_service import _do_settle
        from accounts.wallet_service import InsufficientFunds
        ...
        call_kwargs = ride.save.call_args[1]
        uf = call_kwargs.get("update_fields", [])
        # Only real model field names
        from accounts.models import RideRequest
        real_fields = {f.name for f in RideRequest._meta.concrete_fields}
        for field in uf:
            self.assertIn(field, real_fields, f"'{field}' is not a concrete field on RideRequest")
