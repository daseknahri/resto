...
class MarketplaceOptionBindingTests(SimpleTestCase):
    ...
    def test_select_related_and_binding_in_source(self):
        ...
        from accounts.views import MarketplacePlaceOrderView
        from menu.order_service import resolve_option_map, price_line_options
        ...
        # The binding validation (reject cross-dish/foreign ids) + delta-over-validated-set moved into
        # the shared price_line_options helper (RISK STRUCT-1, slice 2); the view delegates to it.
        self.assertIn("price_line_options", src)
        _bind_src = inspect.getsource(price_line_options)
        self.assertIn("opt_dish_slug != dish.slug", _bind_src)
        self.assertIn("stale_options", _bind_src)
        self.assertIn("for opt in bound_options", _bind_src)
